...
function getNextFriday(date) {
  const day = date.getDay() === 5 ? 7 : (7 - date.getDay() + 5) % 7;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(
    (day2.getTime() - day1.getTime()) / (24 * 60 * 60 * 1000) + 1
  );
}
...
function isDateInPeriod(date, period) {
  const newDate = new Date(date);
  const stringDate = newDate.toISOString().slice(0, 10);
  return stringDate >= period.start && stringDate <= period.end;
}
...
function formatDate(date) {
  ...
  const newDate = event.toLocaleString('en-US', { timeZone: 'UTC' });
  return newDate;
}
...
function getCountWeekendsInMonth(month, year) {
  const daysOfMonth = getCountDaysInMonth(month, year);
  let weekends = 0;
  for (let i = 1; i <= daysOfMonth; i += 1) {
    const date = new Date(year, month - 1, i);
    const dayOfWeek = date.getDay();
    if (dayOfWeek === 6 || dayOfWeek === 0) weekends += 1;
  }
  return weekends;
}
...
function getWeekNumberByDate(date) {
  ...
  const januaryFourth = new Date(currentDate.getFullYear(), 0, 4);
  const daysDifference = (currentDate - januaryFourth) / 86400000;
  const weekNumber = 1 + Math.ceil(daysDifference / 7);
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  newDate.setDate(13);
  while (newDate <= date || newDate.getDay() !== 5) {
